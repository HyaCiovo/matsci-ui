# mp-react-components vs mp-react18-components

本文件用于快速回答“旧库 mp-react-components 与新库 mp-react18-components 差在哪”。定位是“迁移/集成决策文档”，不是逐行代码 diff。
...
- 新库（mp-react18-components）：以 `npm run build` 后的 `dist/index.d.ts` 的 `export { ... }` 作为公开 API。
